import React from "react";
import styles from "../../styles/styles";
import { useSelector } from "react-redux";
import NoEvent from "../../Assests/img/NotFound.svg";
import Loader from "../UI/Loader";
import { selectPromoLoading, selectPromoProducts } from "../../features/product/productSlice";
import ProductCard from "../Route/ProductCard/ProductCard";

const Events = () => {
  const promoProducts = useSelector(selectPromoProducts);
  const promoProductsLoading = useSelector(selectPromoLoading);

  const products = promoProducts.slice(0, 4);

  return (
    <div className={`${styles.section} border-b py-4 mb-10`}>
      <div>
        <div className={`${styles.heading}`}>
          <h1>Promo-Sales</h1>
        </div>
        {promoProductsLoading ? (
          <div className="flex items-center justify-center h-[70vh]">
            <Loader />
          </div>
        ) : (
          <>
            {products && products.length > 0 ? (
              <div className="grid grid-cols-2 gap-[20px] md:grid-cols-3 md:gap-[30px] xl:grid-cols-4 lg:gap-[40px] xl:gap-[50px] 3xl:grid-cols-5 justify-items-center">
                {products.map((product) => (
                  <ProductCard product={product} key={product._id} />
                ))}
              </div>
            ) : (
              <div className="w-full text-center pb-4 flex flex-col items-center justify-center h-[50vh]">
               
                  <img src={NoEvent} alt="Not Found" className="max-h-[250px]" />
              
                <p className="text-[16px] 800px:text-[20px] font-semibold">No Promo-Sales Today!</p>
              </div>
            )}
          </>
        )}
      </div>
    </div>
  );
};

export default Events;
