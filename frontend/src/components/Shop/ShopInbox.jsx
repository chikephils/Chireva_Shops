...
const ShopInbox = () => {
  ...
  const userId = useMemo(() => currentChat?.members?.find((id) => id !== me), [currentChat, me]);
  ...
  // Send text (update list preview on success)
      ...
      <div className="sticky top-0 h-14 bg-slate-500 z-50 flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-3">
          {loadingUser ? (
            ...
            <>
              <img src={user?.avatar?.url} alt="avatar" className="w-10 h-10 rounded-full border border-black object-cover" />
              <div>
                <h1 className="text-base font-semibold text-white">{user?.firstName || "User"}</h1>
